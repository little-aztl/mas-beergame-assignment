...
class DQNAgent:
    def __init__(
        self, state_size, action_size, firm_id,
        max_order=20, buffer_size=10000, batch_size=64, gamma=0.99,
    ...
    def learn(self, experiences):
        ...
        # 转换为torch张量
        states = torch.from_numpy(np.vstack([s.flatten() for s in states])).float() # (B, 3) # 这里的 state_size 是 3，其中 N 是企业的数量
        actions = torch.from_numpy(np.vstack([a-1 for a in actions])).long()  # -1 因为我们的动作从1开始，但索引从0开始 (B, 1)
        rewards = torch.from_numpy(np.vstack(rewards)).float() # (B, 1)
        next_states = torch.from_numpy(np.vstack([ns.flatten() for ns in next_states])).float() # (B, 3)
        dones = torch.from_numpy(np.vstack(dones).astype(np.uint8)).float() # (B, 1)

        # 从目标网络获取下一个状态的最大预测Q值
        Q_targets_next = self.target_network(next_states).detach().max(1)[0].unsqueeze(1) # (B, 1)
